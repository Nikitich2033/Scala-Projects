...
def occurrences(xs: List[String]): Map[String, Int] = {
    ...
    xs.foldLeft(Map.empty[String, Int]){
...
def prod(lst1: List[String], lst2: List[String]) : Int = {
    ...
    val occurrences1 = occurrences(lst1)

    val occurrences2 = occurrences(lst2)

    val listOfProducts = for((val1, num1)<- occurrences1; 
                                if(occurrences2.getOrElse(val1,0) != 0 )) yield {

                       occurrences2.getOrElse(val1,0) * num1;
...
def overlap(lst1: List[String], lst2: List[String]) : Double = {
    prod(lst1,lst2).toDouble/(List(prod(lst1,lst1),prod(lst2,lst2)).max)
}
